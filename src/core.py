# FILE: core.py
# Version: 0.7.11 (Dobavlena realizacija OP_GET_TUPLE_ELEM_X)
...
# --- Version ---
# IZMENENO: Versija obnovlena
CORE_VERSION = "0.7.11" # Dobavlena realizacija OP_GET_TUPLE_ELEM_X
...
# --- Ozhidaemye versii zavisimostej ---
...
try:
    ...
    from tensors import (
        TENSORS_VERSION, TensorCoordinate, create_tensor, validate_tensor, get_tensor_hash,
...
# --- Handle Import Errors Gracefully ---
    ...
    print(f"-----------------------------------------")
    class VeectorDB: pass; VEECTORDB_VERSION = "dummy"
    class TensorCoordinate: pass; TENSORS_VERSION = "dummy"
    def create_tensor(*a,**kw): return []
    def validate_tensor(t): return False
    def get_tensor_hash(t): return "dummy_hash"
    VeectorDB_defined = False
except Exception as other_e:
    print(f"---!!! FATAL ERROR (Other Exception during Import) !!! ---"); print(f"Specific error: {other_e}"); traceback.print_exc(); print(f"Check imported files for syntax errors."); print(f"----------------------------------------------------------"); VeectorDB_defined = False
...
# >>> IZMENENO: Dobavleny novye meta OP dlja raspakovki kortezha <<<
...
# --- Helper dlja logirovanija tenzorov vnutri core ---
...
class Veector:
    ...
    def __init__(self,
        ...
        self._register_model_specific_ops() # Vyzov novoj funkcii registracii
    ...
    def _register_model_specific_ops(self):
        ...
        try:
            ...
            if isinstance(qwen2_ops_dict, dict):
                self.core_ops.update(qwen2_ops_dict)
                print(f"  Successfully registered {len(qwen2_ops_dict)} operations for Qwen2.")
                registered_count += len(qwen2_ops_dict)
            else: print("  WARN: Found veector_models.qwen2.ops but 'qwen2_operations' dictionary is missing or invalid.")
        except ImportError: print("  INFO: veector_models.qwen2.ops module not found, skipping Qwen2 ops registration.")
        except Exception as e: print(f"  ERROR during Qwen2 ops registration: {e}")

        # --- Zdes' mozhno dobavit' registraciju dlja drugih modelej ---
        if registered_count > 0: print(f"Registered {registered_count} model-specific operations.")

    # --- Helper method for OP_APPLY_ROPE ---
    def _execute_apply_rope(self, current_data, **kw):
        return apply_rope(q=kw.get('q'), k=kw.get('k'), position_ids=kw.get('position_ids'), rope_theta=kw.get('rope_theta', 10000.0), num_heads=kw.get('num_heads'), head_dim=kw.get('head_dim'))

    # --- Helper method for OP_SCALED_DOT_PROD_ATTN ---
    def _execute_sdpa(self, current_data, **kw):
        result = scaled_dot_product_attention(**kw)
        return result

    # --- Logging & Monitoring Methods ---
    def _log_memory(self, stage: str): pass
    ...
    def create_tensor(self, *args, **kwargs) -> List:
        if 'create_tensor' in globals(): return create_tensor(*args, **kwargs)
        else: print("Error: create_tensor function not available."); return []
    def validate_tensor(self, tensor_structure: List) -> bool:
        if 'validate_tensor' in globals(): return validate_tensor(tensor_structure)
        else: print("Error: validate_tensor function not available."); return False
    def get_tensor_hash(self, meta_tuple: Tuple) -> str:
        if 'get_tensor_hash' in globals():
             try: return get_tensor_hash(meta_tuple)
             except ValueError as e: print(f"Error hashing metadata tuple: {e}"); return f"error_hash_{random.random()}"
        else: print("Error: get_tensor_hash function not available."); return f"error_hash_{random.random()}"

    # --- Database Interaction Wrappers ---
    def save_tensor(self, tensor_structure: List) -> Optional[str]:
         tensor_id = None
         try:
             if not self.validate_tensor(tensor_structure): print("Error: Attempted to save invalid tensor list structure."); return None
             meta_dict = get_tensor_metadata(tensor_structure); meta_tuple_to_save = meta_dict.get("_encoded_metadata_v1_")
             data_payload = tensor_structure[5] if len(tensor_structure) == 6 else None
             if not meta_tuple_to_save: print("Error: Cannot find embedded metadata tuple for saving."); return None
             tensor_id = self.db.insert_veector_tensor(meta_tuple_to_save, data_payload)
             return tensor_id
         except Exception as e: print(f"Error during save_tensor wrapper call (tensor_id={tensor_id}): {e}"); traceback.print_exc(); return None
    def load_tensor(self, doc_id: str, load_knowledge: bool = False, use_mmap: bool = True) -> Optional[List]:
         try:
            result_structure = self.db.get_veector_tensor(doc_id, load_knowledge_data=load_knowledge, use_mmap=use_mmap)
            if result_structure is not None and not self.validate_tensor(result_structure): print(f"Warning: load_tensor received invalid list structure from DB for {doc_id}."); return None
            return result_structure
         except Exception as e: print(f"Error during load_tensor wrapper call for {doc_id}: {e}"); return None

    # --- load_knowledge_tensor_data ---
    def load_knowledge_tensor_data(self, knowledge_id: str) -> Optional[Any]:
        ...
        if not loaded_structure or len(loaded_structure) != 6: return None
        try: meta_dict = get_tensor_metadata(loaded_structure); loaded_data = loaded_structure[5]
        except Exception as e: print(f"Error extracting meta/data from loaded structure for {knowledge_id}: {e}"); return None
        if loaded_data is None: return None
        final_data = loaded_data; original_dtype_str = meta_dict.get("dtype", "").lower(); is_int8 = ('int8' in original_dtype_str); should_cache = True
        if is_int8:
            scale = meta_dict.get("quantization_scale")
            if scale is None or not isinstance(scale, (float, int)) or scale <= 0: print(f"  ERROR: Missing/invalid 'quantization_scale' ({scale}) for INT8 tensor {knowledge_id}. Cannot dequantize!"); return None
            try:
                scale_float16 = np.float16(scale)
                if loaded_data.dtype != np.int8: print(f"  WARN: Data for {knowledge_id} marked INT8 but dtype is {loaded_data.dtype}. Attempting cast."); loaded_data = loaded_data.astype(np.int8)
                dequantized_data = loaded_data.astype(np.float16) * scale_float16
                final_data = dequantized_data
                tags = meta_dict.get("tags", [])
                if TAG_COMP_EMBEDDING in tags or TAG_COMP_LM_HEAD in tags: should_cache = False
            except Exception as dequant_e: print(f"  ERROR during dequantization for {knowledge_id}: {dequant_e}"); return None
        if final_data is not None and should_cache: self._add_to_cache(knowledge_id, final_data, is_knowledge=True)
        return final_data

    # --- Caching Methods ---
    def _update_cache_access(self, key: Union[str, Tuple]):
        self.cache_timestamps[key] = time.time(); self.cache_access_count[key] = self.cache_access_count.get(key, 0) + 1
    def _evict_cache(self):
        ...
        num_to_evict = total_items - self.cache_size; all_keys_metrics=[]
        try:
            sorter=lambda k: self.cache_access_count.get(k,0) if self.eviction_strategy=="LFU" else self.cache_timestamps.get(k,0)
            all_keys_metrics=[(k, sorter(k)) for k in compute_keys+knowledge_keys]; all_keys_metrics.sort(key=lambda item: item[1])
    ...
    def _add_to_cache(self, key: Union[str, Tuple], value: Any, is_knowledge: bool = False):
         cache = self.knowledge_cache if is_knowledge else self.compute_cache; cache[key]=value; self._update_cache_access(key); self._evict_cache()
    ...
    # IZMENENO: Dobavlena obrabotka OP_GET_TUPLE_ELEM_X
    def _execute_op_sequence(
        ...
        **kw_context
        ...
        """ Executes op sequence. v0.7.11 - Added tuple unpacking ops. """
        ...
        for i, op_command in enumerate(ops_sequence):
            step_provenance = {"step": i}; step_start = time.time(); op_tuple = None; op_code_list = None; op_func = None;
            op_call_args_from_processor: Dict = {}; meta_args = []; valid_command = False; op_result = None
            is_meta_op_with_args = False

            # --- Parsing command ---
            if not isinstance(op_command, list) or not op_command: error_msg = f"Command at step {i} is not a non-empty list: {op_command}"; step_provenance["error"] = error_msg; step_provenance_list.append(step_provenance); return None, step_provenance_list, step_context
            ...
            if isinstance(op_command[0], int):
                if len(op_command) >= 3 and all(isinstance(x, int) for x in op_command[:3]):
                    op_code_list = op_command[:3]; op_tuple = tuple(op_code_list)
                    if len(op_command) == 4 and isinstance(op_command[3], dict):
                        op_call_args_from_processor = op_command[3]; valid_command = True
                        if op_tuple == tuple(OP_MAKE_TUPLE): is_meta_op_with_args = True
                    elif len(op_command) >= 3:
                        valid_command = True
                        # >>> IZMENENO: Proverjaem vse meta-operacii, vkljuchaja novye <<<
                        is_other_meta_op = op_tuple in (tuple(OP_STORE), tuple(OP_LOAD), tuple(OP_LOAD_INITIAL_INPUT), tuple(OP_DEBUG_CONTEXT), tuple(OP_GET_TUPLE_ELEM_0), tuple(OP_GET_TUPLE_ELEM_1), tuple(OP_GET_TUPLE_ELEM_2))
                        if is_other_meta_op and len(op_command) > 3: meta_args = op_command[3:]
                 ...
                 if len(op_code_container) == 3 and all(isinstance(x, int) for x in op_code_container):
                     op_code_list = op_code_container; op_tuple = tuple(op_code_list)
                     if len(op_command) == 2 and isinstance(op_command[1], dict):
                         op_call_args_from_processor = op_command[1]; valid_command = True
                         if op_tuple == tuple(OP_MAKE_TUPLE): is_meta_op_with_args = True
                     elif len(op_command) >= 1:
                         valid_command = True
                         is_other_meta_op = op_tuple in (tuple(OP_STORE), tuple(OP_LOAD), tuple(OP_LOAD_INITIAL_INPUT), tuple(OP_DEBUG_CONTEXT), tuple(OP_GET_TUPLE_ELEM_0), tuple(OP_GET_TUPLE_ELEM_1), tuple(OP_GET_TUPLE_ELEM_2))
                         if is_other_meta_op and len(op_command) > 1: meta_args = op_command[1:]
            if not valid_command or op_code_list is None: error_msg=f"Cannot parse OP CODE / Invalid command format at step {i}: {op_command}"; step_provenance["error"]=error_msg; step_provenance_list.append(step_provenance); return None, step_provenance_list, step_context
            ...
            # --- Logirovanie PERED operaciej ---
            print(f"--- [CORE DBG] Step {i}: Op={op_tuple}, MetaArgs={meta_args}, OpArgsKeys={list(op_call_args_from_processor.keys())} ---")
            _log_core_tensor_stats(f"Step {i} BEFORE", current_data)

            # --- Execution ---
            is_handled_meta_op = False
            # >>> IZMENENO: Dobavlena obrabotka OP_GET_TUPLE_ELEM_X <<<
            if op_tuple in (tuple(OP_STORE), tuple(OP_LOAD), tuple(OP_LOAD_INITIAL_INPUT), tuple(OP_DEBUG_CONTEXT), tuple(OP_GET_TUPLE_ELEM_0), tuple(OP_GET_TUPLE_ELEM_1), tuple(OP_GET_TUPLE_ELEM_2)):
                is_handled_meta_op = True
                var_name = meta_args[0] if meta_args and isinstance(meta_args[0], str) else None # Dlja STORE/LOAD
                if op_tuple == tuple(OP_STORE):
                    if not var_name: error_msg = f"OP_STORE needs str arg"; step_provenance["error"] = error_msg; step_provenance_list.append(step_provenance); return None, step_provenance_list, step_context
                    step_context[var_name] = current_data
                    print(f"  [CORE DBG] Stored '{var_name}' in step_context.")
                    _log_core_tensor_stats(f"Step {i} AFTER STORE", current_data)
                elif op_tuple == tuple(OP_LOAD):
                    if not var_name: error_msg = f"OP_LOAD needs str arg"; step_provenance["error"] = error_msg; step_provenance_list.append(step_provenance); return None, step_provenance_list, step_context
                    if var_name in step_context: current_data = step_context[var_name]; _log_core_tensor_stats(f"Step {i} AFTER LOAD '{var_name}'", current_data)
                    else: error_msg = f"LOAD failed: Var '{var_name}' not in context"; step_provenance["error"] = error_msg; step_provenance_list.append(step_provenance); return None, step_provenance_list, step_context
                elif op_tuple == tuple(OP_LOAD_INITIAL_INPUT): current_data = step_context.get('_initial_input'); _log_core_tensor_stats(f"Step {i} AFTER LOAD_INITIAL", current_data)
                elif op_tuple == tuple(OP_DEBUG_CONTEXT): print(f"  [CORE DBG] DEBUG_CONTEXT: Keys={list(step_context.keys())}"); _log_core_tensor_stats(f"Step {i} AFTER DEBUG", current_data)
                # Obrabotka novyh meta-operacij
                elif op_tuple == tuple(OP_GET_TUPLE_ELEM_0):
                    if isinstance(current_data, tuple) and len(current_data) > 0: current_data = current_data[0]; _log_core_tensor_stats(f"Step {i} AFTER GET_TUPLE_ELEM_0", current_data)
                    else: error_msg = f"GET_TUPLE_ELEM_0 failed: current_data is not a tuple or is empty (type: {type(current_data)})"; step_provenance["error"] = error_msg; step_provenance_list.append(step_provenance); return None, step_provenance_list, step_context
                elif op_tuple == tuple(OP_GET_TUPLE_ELEM_1):
                    if isinstance(current_data, tuple) and len(current_data) > 1: current_data = current_data[1]; _log_core_tensor_stats(f"Step {i} AFTER GET_TUPLE_ELEM_1", current_data)
                    else: error_msg = f"GET_TUPLE_ELEM_1 failed: current_data is not a tuple or too short (type: {type(current_data)}, len: {len(current_data) if isinstance(current_data, tuple) else 'N/A'})"; step_provenance["error"] = error_msg; step_provenance_list.append(step_provenance); return None, step_provenance_list, step_context
                elif op_tuple == tuple(OP_GET_TUPLE_ELEM_2):
                    if isinstance(current_data, tuple) and len(current_data) > 2: current_data = current_data[2]; _log_core_tensor_stats(f"Step {i} AFTER GET_TUPLE_ELEM_2", current_data)
                    else: error_msg = f"GET_TUPLE_ELEM_2 failed: current_data is not a tuple or too short (type: {type(current_data)}, len: {len(current_data) if isinstance(current_data, tuple) else 'N/A'})"; step_provenance["error"] = error_msg; step_provenance_list.append(step_provenance); return None, step_provenance_list, step_context

            if not is_handled_meta_op: # Eto libo obychnaja operacija, libo OP_MAKE_TUPLE
                op_func = self.core_ops.get(op_tuple)
                if not op_func: error_msg = f"Op {op_tuple} not found"; step_provenance["error"] = error_msg; step_provenance_list.append(step_provenance); return None, step_provenance_list, step_context
                resolved_args = {}
                ...
                op_kw = {**kw_context, **resolved_args}
                try:
                    op_result = op_func(current_data, **op_kw)
                    # Spesial'naja obrabotka dlja OP_MAKE_TUPLE (esli on ispol'zuetsja)
                    if op_tuple == tuple(OP_MAKE_TUPLE):
                        if isinstance(op_result, tuple) and len(op_result) == 3:
                            step_context['k_cache_out'] = op_result[1]; step_context['v_cache_out'] = op_result[2]
                            print(f"  [CORE DBG] Stored 'k_cache_out' (Type: {type(op_result[1])})"); print(f"  [CORE DBG] Stored 'v_cache_out' (Type: {type(op_result[2])})")
                            current_data = op_result[0]
                            _log_core_tensor_stats(f"Step {i} AFTER MAKE_TUPLE (Output Element)", current_data)
                        else: current_data = op_result; _log_core_tensor_stats(f"Step {i} AFTER MAKE_TUPLE (Unexpected Result)", current_data)
                    else: # Dlja vseh ostal'nyh (vkljuchaja novye vysokourovnevye)
                        current_data = op_result; _log_core_tensor_stats(f"Step {i} AFTER", current_data)
                except Exception as e: error_msg = f"Op {op_tuple} at step {i} failed: {e}"; step_provenance["error"] = str(e); print(f"--- Exception during Op {op_tuple} execution (Step {i}) ---"); traceback.print_exc(); print(f"--- End Exception ---"); return None, step_provenance_list, step_context
            # --- End Execution ---

            step_provenance["duration_ms"] = (time.time() - step_start) * 1000; step_provenance_list.append(step_provenance)
            if isinstance(current_data, dict) and current_data.get('status') == 'error': error_msg = f"Op {op_tuple} returned error status: {current_data.get('error')}"; step_provenance["error"] = error_msg; return None, step_provenance_list, step_context
            # --- End Step Finish ---

        return current_data, step_provenance_list, step_context

    # --- Knowledge Selection (v0.7.9) ---
    def _select_knowledge_tensors(self, processor_structure: List, context: Dict) -> Dict[str, str]:
        ...
        try:
            ...
            if not processor_coord: raise ValueError("Processor coordinates not found")
        except Exception as e: print(f"Error selecting knowledge: Cannot access metadata: {e}"); return {}
        ...
        print(f"  [SelectKnowledge] Received context: target_group={target_knowledge_group_from_context}, required_nest={required_nest}")
        ...
        if target_knowledge_group_from_context is not None:
            ...
            print(f"  [SelectKnowledge] Using target_group from context: {final_target_group}")
        else:
            print(f"  [SelectKnowledge] target_group not in context, trying to infer from processor tags: {processor_tags_list}")
            model_tag = None
            ...
            if final_target_group is None: print(f"  [SelectKnowledge] WARN: Could not infer target knowledge group from processor tags. Searching without group filter.")
            else: print(f"  [SelectKnowledge] Inferred group {final_target_group} from processor tag {model_tag}")
        target_nest = required_nest if isinstance(required_nest, int) else processor_coord.nest
        print(f"  [SelectKnowledge] Final search parameters: Group={final_target_group}, Nest={target_nest}")
        db_coord_filter = {};
        ...
        print(f"  [SelectKnowledge] Found {len(all_candidate_structures)} candidate knowledge tensors in G={final_target_group}, N={target_nest}.")
        for need in knowledge_needs:
            ...
            for cand_id, cand_structure in all_candidate_structures.items():
                 try:
                     if not self.validate_tensor(cand_structure): continue
                     tensor_tags_list = get_tensor_tags(cand_structure)
                     if query_tags_set.issubset(set(tensor_tags_list)): best_candidate_id = cand_id; found_match_for_need = True; break
            ...
            elif not is_optional: err_msg = f"Missing REQUIRED knowledge for '{param_name}' with tags {query_tags_set} in G={final_target_group}, N={target_nest}. Candidates checked: {len(all_candidate_structures)}."; print(f"ERROR: {err_msg}"); raise ValueError(err_msg)
        print(f"  [SelectKnowledge] Selection complete. Selected map size: {len(selected_knowledge_map)}")
    ...
    def compute(self, processor_id: str, context: Optional[Dict] = None) -> Dict:
        start_time = time.time(); context = context if context is not None else {}; provenance = { "processor_id": processor_id, "steps": [], "timestamp_start": datetime.now().isoformat(), "context_received": {k: v for k, v in context.items() if k != 'input_data'} }; final_step_context = {}
        ...
        state_id = context.get("state_id"); required_nest = context.get("required_nest", "default"); cache_key = (processor_id, required_nest, state_id if state_id else input_hash_str)
        if cache_key in self.compute_cache: cached_data = self.compute_cache[cache_key]; self._update_cache_access(cache_key); provenance["status"] = "cached"; provenance["timestamp_end"] = datetime.now().isoformat(); provenance["duration_ms"] = (time.time() - start_time) * 1000; return {"data": cached_data, "provenance": provenance, "status": "completed", "step_context": {}}
        processor_structure = self.load_tensor(processor_id, load_knowledge=False)
        if not processor_structure: error_msg = f"Processor {processor_id} structure list not found/invalid."; provenance["error"] = error_msg; provenance["status"] = "error"; return {"data": None, "provenance": provenance, "status": "error", "step_context": {}}
        try:
            if not self.validate_tensor(processor_structure): raise ValueError("Invalid processor structure list.")
            tensor_type_str = get_tensor_type(processor_structure); status_str = get_tensor_status(processor_structure); coord_obj = get_tensor_coord(processor_structure); meta_dict = get_tensor_metadata(processor_structure); evo_version = meta_dict.get("evolutionary_version", 1)
        ...
        except Exception as e: error_msg = f"Error processing loaded structure for {processor_id}: {e}"; provenance["error"] = error_msg; provenance["status"] = "error"; return {"data": None, "provenance": provenance, "status": "error", "step_context": {}}
        ops_sequences_dict = get_processor_ops_sequences(processor_structure) or {}; precision_key = 'default';
        ...
        if not ops_sequence: result_data = context.get("input_data"); provenance["status"] = "completed (no ops)"; provenance["timestamp_end"] = datetime.now().isoformat(); provenance["duration_ms"] = (time.time() - start_time) * 1000; self._add_to_cache(cache_key, result_data); return {"data": result_data, "provenance": provenance, "status": "completed", "step_context": {}}
        ...
        try:
            ...
            for param_name, knowledge_id in selected_knowledge_map.items():
                 ...
                 if data is None: raise ValueError(f"Failed load knowledge blob {knowledge_id} (for param '{param_name}')")
                 loaded_knowledge_by_param_name[param_name] = data; knowledge_ids_loaded.append(knowledge_id)
        except Exception as load_err: error_msg = f"Knowledge select/load fail: {load_err}"; provenance["error"] = error_msg; provenance["status"] = "error"; return {"data": None, "provenance": provenance, "status": "error", "step_context": {}}
        ...
        if state_id_used:
             ...
             if not isinstance(intermediate_state_data, dict): error_msg = f"Invalid state data format for {state_id_used}"; provenance["error"] = error_msg; provenance["status"] = "error"; return {"data": None, "provenance": provenance, "status": "error", "step_context": {}}
             current_data = intermediate_state_data.get("data"); current_step = intermediate_state_data.get("next_step", 0); initial_step_context_from_state = intermediate_state_data.get("step_context", {}); provenance["resumed_from_state"] = state_id_used; provenance["resumed_from_step"] = current_step;
        ...
        max_steps_per_call = context.get("max_steps", len(ops_sequence)); steps_executed_this_call = 0; exec_start_time = time.time(); ops_to_execute = []; result_data = None; step_provenance_list = []
        if 0 <= current_step < len(ops_sequence): end_step = min(current_step + max_steps_per_call, len(ops_sequence)); ops_to_execute = ops_sequence[current_step : end_step]
        if ops_to_execute:
            try:
                ...
                # Proverjaem oshibku v poslednem shage proverki
                if step_provenance_list and "error" in step_provenance_list[-1]:
                     error_msg = f"Op sequence failed at step {current_step + steps_executed_this_call - 1}: {step_provenance_list[-1].get('error','Unknown')}"
            ...
            except Exception as exec_e: error_msg = f"Error during op sequence exec: {exec_e}"; provenance["error"] = error_msg; provenance["status"] = "error"; traceback.print_exc(); return {"data": None, "provenance": provenance, "status": "error", "step_context": final_step_context}
        ...
        if current_step < len(ops_sequence):
            state_to_save = {"data": result_data, "next_step": current_step, "step_context": final_step_context}; state_coord = TensorCoordinate( layer=STATE_TENSOR_LAYER, group=coord_obj.group, nest=coord_obj.nest, x=random.randint(10000,99999), y=coord_obj.y, z=coord_obj.z )
            try:
                state_tensor_structure = self.create_tensor( coord=state_coord, tensor_type="state", knowledge_data=state_to_save, status="temporary", parents=[processor_id] ); state_id_new = self.save_tensor(state_tensor_structure)
            ...
            except Exception as state_e: error_msg = f"Failed create/save state: {state_e}"; provenance["error"] = error_msg; provenance["status"] = "error"; return {"data": None, "provenance": provenance, "status": "error", "step_context": final_step_context}
        ...
        self._add_to_cache(cache_key, result_data);
        # >>> IZMENENO: Vsegda vozvrashhaem final_step_context <<<
        # Dazhe esli poslednej operaciej ne byla OP_MAKE_TUPLE, kontekst mozhet soderzhat' kesh posle OP_QWEN2_ATTENTION
        return {"data": result_data, "provenance": provenance, "status": "completed", "step_context": final_step_context}

    # --- Spawning / Evolution (bez izmenenij) ---
    def spawn_tensor(self, parent_id: str, strategy: str = "inherit", context: Optional[Dict] = None) -> Optional[str]:
        ...
        if not parent_structure or not self.validate_tensor(parent_structure): print(f"Error spawn: Parent {parent_id} not found or invalid."); return None
        try:
            parent_type_str = get_tensor_type(parent_structure); parent_coord_obj = get_tensor_coord(parent_structure); parent_tags = get_tensor_tags(parent_structure); parent_interface = get_tensor_interface(parent_structure); parent_ops_sequences = get_processor_ops_sequences(parent_structure); parent_filters = get_tensor_filters(parent_structure); parent_exit_gates = get_tensor_exit_gates(parent_structure); parent_meta_dict = get_tensor_metadata(parent_structure); parent_dtype_str = parent_meta_dict.get("dtype"); parent_shape = parent_meta_dict.get("shape");
        ...
        child_coord = self._generate_next_coords(parent_coord_obj); child_metadata_extra = {"spawn_strategy": strategy}; child_type = parent_type_str; child_knowledge_data = None; child_tags = list(parent_tags); child_interface = parent_interface.copy() if parent_interface else None; child_ops_sequences = parent_ops_sequences.copy() if parent_ops_sequences else None; child_filters = list(parent_filters) if parent_filters else None; child_exit_gates = list(parent_exit_gates) if parent_exit_gates else None; child_dtype = parent_dtype_str; child_shape = parent_shape
        try:
            ...
            if parent_type_str == "knowledge":
                if strategy == "inherit" or not parent_has_blob: child_knowledge_data = None
                    ...
                    if parent_data is not None and isinstance(parent_data, np.ndarray): scale = context.get("mutation_scale", 0.05) if context else 0.05; noise = np.random.normal(0, scale, parent_data.shape).astype(parent_data.dtype); child_knowledge_data = parent_data + noise; child_dtype = child_knowledge_data.dtype; child_shape = child_knowledge_data.shape; child_tags = [t for t in child_tags if not (20 <= t <= 29)]; new_prec_tag = DTYPE_MAPPING.get(child_dtype, TAG_PREC_FLOAT16); child_tags.append(new_prec_tag)
                    ...
                    if parent_data is not None:
                        ...
                        if np.issubdtype(parent_data.dtype, np.floating): abs_max = np.max(np.abs(parent_data)); scale = abs_max / 127.0 if abs_max > 1e-9 else 1.0; scale = max(scale, 1e-9); distilled_data = np.round(parent_data / scale).astype(np.int8); child_knowledge_data = distilled_data; child_dtype = np.int8; child_shape = distilled_data.shape; new_prec_tag = TAG_PREC_INT8; child_tags = [t for t in child_tags if not (20 <= t <= 29)]; child_tags.append(new_prec_tag); child_metadata_extra["quantization_scale"] = float(scale)
        ...
        try:
            child_tensor_structure = self.create_tensor( coord=child_coord, tensor_type=child_type, tags=list(set(child_tags)), interface=child_interface, ops_sequences=child_ops_sequences, filters=child_filters, exit_gates=child_exit_gates, knowledge_data=child_knowledge_data, dtype=child_dtype, shape=child_shape, name_id=-1, parents=[parent_id], evolutionary_version=1, status="active", metadata_extra=child_metadata_extra ); child_id = self.save_tensor(child_tensor_structure)
    ...
    # --- Vspomogatel'nye i special'nye metody (bez izmenenij) ---
    def _distill_knowledge_data(self, data: Any, target_format: str) -> Any:
        ...
        return data
    def _generate_next_coords(self, c: TensorCoordinate) -> TensorCoordinate: return TensorCoordinate( layer=c.layer, group=c.group, nest=c.nest, x=c.x, y=c.y, z=c.z + 1 )
    def _op_conditional_if(self, data: Any, **kw) -> Any: c=bool(data[0]) if isinstance(data,(list,np.ndarray)) and len(data)>0 else bool(data);t=data[1] if isinstance(data,(list,np.ndarray)) and len(data)>1 else None;f=data[2] if isinstance(data,(list,np.ndarray)) and len(data)>2 else None;return t if c else f
    ...
    def _op_choice_select(self, data: Any, **kw) -> Any: i=kw.get('index', 0);o=kw.get('options', []); return o[i] if isinstance(o, list) and 0<=i<len(o) else None
    def _op_trigger_reason(self, data: Any, **kw) -> Any: return data
    def _op_graph_dfs(self, data: Any, **kw) -> Any: return []
    def _op_output_print(self, data: Any, **kw): label = kw.get("label", "PRINT"); print(f"--- {label} (Step Data) ---"); print(data); print(f"--- End {label} ---"); return data
    def _quantum_op_placeholder(self, data: Any, op_type: str, **kw) -> Any: print(f"WARN: Quantum op '{op_type}' called, but Qiskit not fully integrated."); return data
    def _get_tensor_manager(self) -> Optional[Any]: print("Warning: _get_tensor_manager called, not implemented."); return None
    def handle_task( self, task_description: Any, input_data: Any, resources: Dict, priority: float ) -> Optional[Dict]: print("Warning: handle_task called, but TensorManager is not implemented."); return None
