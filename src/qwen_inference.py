...
# Version: 0.2.15 (Updated core requirement)
...
# --- Version ---
QWEN_INFERENCE_VERSION = "0.2.15" # Added prompt template, sampling params, fixed group ID
...
CORE_VERSION_REQ = "0.6.13"
TENSORS_VERSION_REQ = "0.7.6"
VEECTORDB_VERSION_REQ = "0.9.7"
OPERATIONS_VERSION_REQ = "0.7.3"# Обновлено для полноты
...
try:
    ...
    # ... (импорт Tensors, проверка TENSORS_VERSION_REQ) ...
    ...
    # <<< Обновлена проверка версии operations >>>
...
def run_qwen_inference(
    ...
    try:
        ...
        # --- Конец Цикла по Слоям ---
            print(f"  Clearing knowledge cache and running GC after Layer {layer_idx}...")
            if vec:
                vec.clear_cache(clear_knowledge=True, clear_compute=False) # Очищаем только кэш знаний
