...
class QuantizedModelTest:
    ...
    def process_layer(self, hidden_states, layer_idx, past_kv=None):
        ...
        # Исправленные размеры для внимания
        q = self.virtual_space.virtual_matrix.linear(
            hidden_states, f"layers.{layer_idx}.attn_q",
            self.key_dim * self.num_key_value_heads, self.hidden_size  # 256 * 2 = 512
        ).to(self.dtype)
        ...
        head_dim = self.hidden_size // self.num_attention_heads  # 128
        key_head_dim = self.key_dim // self.num_key_value_heads  # 128
        heads_per_group = self.num_attention_heads // self.num_key_value_heads  # 6
        
        q = q.view(batch_size, seq_len, self.num_key_value_heads, heads_per_group, key_head_dim).permute(0, 2, 1, 3, 4)
        ...
        attn_output = torch.zeros(batch_size, seq_len, self.num_key_value_heads, heads_per_group, key_head_dim, 
                                 device=self.device, dtype=self.dtype)
        for group_idx in range(self.num_key_value_heads):
            q_group = q[:, group_idx]
            k_group = k[:, group_idx]
            v_group = v[:, group_idx]
            scores = torch.matmul(q_group, k_group.transpose(-2, -1)) / (key_head_dim ** 0.5)
    ...
    def process_input(self, text):
        try:
            ...
            with torch.no_grad():
                ...
                hidden_states = self.virtual_space.virtual_matrix.embedding(input_ids, "token_embd")
                logger.info(f"Эмбеддинги загружены за {(time.time() - start_time)*1000:.2f} мс, размер: {hidden_states.nbytes / 1024**2:.2f} MiB")
                ...
                for layer_idx in range(self.num_layers):
                    ...
                    logger.info(f"Слой {layer_idx} обработан за {(time.time() - layer_start)*1000:.2f} мс")
                    past_kv_cache[layer_idx] = past_kv
                
                # Генерация (добавим замеры)
                generated_ids = input_ids.clone()
                for step in range(self.max_length):
                    gen_start = time.time()
                    hidden_states = self.norm(hidden_states)
                    logits, coarse_indices = self.virtual_space.virtual_matrix.linear(
                        hidden_states, "output", self.vocab_size, self.hidden_size, top_k=self.top_k
                    )
                    next_token = self._sample_next_token(logits, coarse_indices)
                    logger.debug(f"Шаг {step}: токен сгенерирован за {(time.time() - gen_start)*1000:.2f} мс")
                    generated_ids = torch.cat([generated_ids, next_token], dim=-1)
                    hidden_states = self.virtual_space.virtual_matrix.embedding(next_token, "token_embd")
                    # Обновляем KV-кэш
                    for layer_idx in range(self.num_layers):
                        hidden_states, past_kv = self.process_layer(hidden_states, layer_idx, past_kv_cache[layer_idx])
                        past_kv_cache[layer_idx] = past_kv
