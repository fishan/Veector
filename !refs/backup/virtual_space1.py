...
class VirtualMatrix:
    ...
    def linear(self, input, prefix, output_size, input_size, top_k=None):
        ...
        if prefix.endswith("_output") and top_k is not None:
            coarse_logits = torch.zeros(batch_size, seq_len, 4096 * 2, dtype=torch.float16, device=self.device)
            for block_key in block_keys[:2]:
                block = self.get_block(block_key)
...
class ModelDispatcher:
    ...
    def get_layer_blocks(self, layer_idx, component):
        ...
        for block_key, info in self.metadata.items():
            if f"layer{layer_idx}_{component}" in block_key:
                blocks.add(block_key)
        if not blocks:
            logger.warning(f"Не найдено блоков для layer_idx={layer_idx}, component={component}")
        return blocks
    ...
    def assemble_layer_tensor(self, layer_idx, component, target_shape):
        block_keys = self.get_layer_blocks(layer_idx, component)
        return self.assemble_tensor(block_keys, target_shape)
...
class MatrixModel(nn.Module):
    ...
    def forward(self, input_ids, top_k=None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            del q, k, v, attn_weights, attn_output, gate, up, mlp_output
            torch.cuda.empty_cache()
...
class VirtualSpace:
    ...
    def switch_model(self, model_name: str, vocab_size: int, hidden_size: int, num_layers: int, num_attention_heads: int, intermediate_size: int, key_dim: int, num_key_value_heads: int = 2):
        ...
        self.matrix_models[model_name] = MatrixModel(self.dispatcher)  # Возвращаем MatrixModel
        logger.info(f"Переключено на модель: {model_name}")

    def get_matrix(self):
        if not self.current_model:
            raise ValueError("Не выбрана активная модель")
        return self.matrix_models[self.current_model]
...
if __name__ == "__main__":
    ...
    logits, indices = matrix(input_ids)
