...
class Observer:
    ...
    def process_layer(self, hidden_states, layer_idx, input_ids):
        ...
        # Фильтрация с порогом 0.15
        attn_weights_filtered = torch.where(attn_weights < 0.125, torch.tensor(0.0, device=self.device), attn_weights)
        attn_weights_filtered = attn_weights_filtered / attn_weights_filtered.sum(dim=-1, keepdim=True)  # Нормализация

        # Логируем топ-5 после фильтрации
        top_k = 5
        top_weights, top_indices = torch.topk(attn_weights_filtered, k=top_k, dim=-1)
        attended_tokens = [[input_ids[0, idx].item() for idx in head_indices[0]] for head_indices in top_indices[0]]

        # Используем отфильтрованные веса для вывода
        attn_output = torch.matmul(attn_weights_filtered, v)
    ...
    def generate(self, input_text, max_length=50, temperature=0.7):
        ...
        for layer_idx in range(min(self.dispatcher.num_layers, 3)):
            hidden_states, attended_tokens = self.process_layer(hidden_states, layer_idx, input_ids)
            attended_text = [[self.tokenizer.decode([token], skip_special_tokens=True) for token in tokens] for tokens in attended_tokens]
            logger.info(f"Layer {layer_idx} attended tokens per head: {attended_text}")
            print(f"Layer {layer_idx} attended tokens per head: {attended_text}")
        ...
        print(f"Final output after 3 layers: {final_text}")
        return final_text
