...
files = os.listdir(model_dir)  # Убираем фильтр, берём все файлы
...
with zipfile.ZipFile(zip_name, 'w', zipfile.ZIP_DEFLATED) as zipf:
    for f in files:  # Используем тот же список всех файлов
        zipf.write(os.path.join(model_dir, f), arcname=f)
print(f"🟢 [LOG] 📦 Архив создан: {zip_name}, размер: {os.path.getsize(zip_name) / (1024 * 1024):.2f} MB {log_memory()}")
#--------------------------------------------
...
model_path = hf_hub_download(repo_id=model_repo, filename=model_filename)
print(f"🟢 [LOG] ✅ Модель загружена: {model_path} {log_memory()}")
...
os.makedirs(model_dir, exist_ok=True)
split_model_path = os.path.join(model_dir, "model_split.onnx")
print(f"🟢 [LOG] Создана папка: {model_dir} {log_memory()}")

#--------------------------------------------
# Разбиваем модель
chunk_size = 1024 * 1024 * 50  # 50 MB
model = onnx.load(model_path)
print(f"🟢 [LOG] Модель прочитана в память {log_memory()}")
onnx.save_model(model, split_model_path, save_as_external_data=True, all_tensors_to_one_file=False, size_threshold=chunk_size)
print(f"🟢 [LOG] Модель разбита и сохранена: {split_model_path} {log_memory()}")
...
for f in files:
    size_mb = os.path.getsize(os.path.join(model_dir, f)) / (1024 * 1024)
    print(f"🟢 [LOG] 📄 {f}: {size_mb:.2f} MB")
print(f"🟢 [LOG] Файлы проверены {log_memory()}")

#--------------------------------------------
# Загрузка модели из папки
os.environ["ONNX_LOAD_EXTERNAL_LOGGING"] = "1"
onnx_model = onnx.load(split_model_path, load_external_data=False)
print(f"🟢 [LOG] Модель загружена без внешних данных {log_memory()}")
load_external_data_for_model(onnx_model, model_dir)
print(f"🟢 [LOG] Внешние данные подгружены из {model_dir} {log_memory()}")
onnx.save(onnx_model, split_model_path)
print(f"🟢 [LOG] Модель сохранена с данными {log_memory()}")
session = ort.InferenceSession(split_model_path)
print(f"🟢 [LOG] ✅ ONNX Runtime загружен. Входы модели: {[inp.name for inp in session.get_inputs()]} {log_memory()}")
...
def preprocess_text(text):
    ...
    for i in range(num_hidden_layers):
        input_feed[f"past_key_values.{i}.key"] = np.zeros((1, num_key_value_heads, 0, head_dim), dtype=np.float32)
        input_feed[f"past_key_values.{i}.value"] = np.zeros((1, num_key_value_heads, 0, head_dim), dtype=np.float32)
    print(f"🟢 [LOG] Вход подготовлен: input_ids shape={inputs['input_ids'].shape} {log_memory()}")
    return input_feed, inputs["input_ids"], inputs["attention_mask"]
...
def generate_text(input_feed, input_ids, attention_mask, max_new_tokens):
    ...
    generated_ids.extend(think_tokens)
    print(f"🟢 [STREAM] {tokenizer.decode(think_tokens, skip_special_tokens=False)}", end='', flush=True)
    print(f"🟢 [LOG] Добавлен <think> {log_memory()}")
    ...
    generated_ids.append(next_token)
    print(f"🟢 [STREAM] {tokenizer.decode([next_token], skip_special_tokens=False)}", end='', flush=True)
    print(f"🟢 [LOG] Первый токен сгенерирован {log_memory()}")

    # Обновляем past_key_values
    for i in range(num_hidden_layers):
        past_key_values[f"past_key_values.{i}.key"] = outputs[2 * i + 1]
        past_key_values[f"past_key_values.{i}.value"] = outputs[2 * i + 2]
        print(f"🟢 [LOG] past_key_values.{i}.key shape={past_key_values[f'past_key_values.{i}.key'].shape} {log_memory()}")
    ...
    for step in range(max_new_tokens - 1):
        ...
        print(f"🟢 [LOG] Шаг {step + 1}: input_ids shape={input_feed['input_ids'].shape}, attention_mask shape={input_feed['attention_mask'].shape} {log_memory()}")
        outputs = session.run(None, input_feed)
        logits = outputs[0][:, -1, :]
        logits = logits / temperature
        probs = np.exp(logits) / np.sum(np.exp(logits), axis=-1, keepdims=True)
        ...
        generated_ids.append(next_token)
        print(f"🟢 [STREAM] {tokenizer.decode([next_token], skip_special_tokens=False)}", end='', flush=True)
        print(f"🟢 [LOG] Токен {step + 1} сгенерирован {log_memory()}")
    ...
    print()
    print(f"🟢 [LOG] Генерация завершена {log_memory()}")
    return tokenizer.decode(generated_ids, skip_special_tokens=True).strip()
...
def chat():
    ...
    while True:
        user_input = input("Ты: ")
        if user_input.lower() == "выход":
            print(f"🤖 Чат завершен. {log_memory()}")
            break
...
# Выгрузка на Google Drive
drive.mount('/content/drive', force_remount=True)
print(f"🟢 [LOG] Google Drive подключён {log_memory()}")
destination_path = f"/content/drive/My Drive/{zip_name}"
shutil.copy(zip_name, destination_path)
print(f"🟢 [LOG] ✅ Архив загружен на Google Drive: {destination_path} {log_memory()}")
